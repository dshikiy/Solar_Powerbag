...
function renderOrders() {
    ...
    // Try backend first
    fetch('/api/orders').then(r => r.ok ? r.json() : Promise.reject()).then(orders => {
        if (!orders || !orders.length) { el.innerHTML = '<p>No orders yet.</p>'; return; }
        el.innerHTML = '';
        orders.slice().reverse().forEach(order => {
            const wrap = document.createElement('div');
            wrap.style.borderBottom = '1px solid #eee';
            wrap.style.padding = '10px 0';
            wrap.innerHTML = `<div style="display:flex; justify-content:space-between; align-items:center;"><div><strong>Order #${order.id}</strong><div style="color:#666; font-size:13px;">${new Date(order.date).toLocaleString()}</div></div></div>`;
        ...
        orders.slice().reverse().forEach(order => {
            ...
            wrap.style.borderBottom = '1px solid #eee';
            wrap.style.padding = '10px 0';
            wrap.innerHTML = `<div style="display:flex; justify-content:space-between; align-items:center;"><div><strong>Order #${order.id}${order.offline? ' (offline)':''}</strong><div style="color:#666; font-size:13px;">${new Date(order.date).toLocaleString()}</div></div><div><button class="btn-primary btn-small">Delete</button></div></div>`;
...
function renderMessages() {
    ...
    fetch('/api/messages').then(r => r.ok ? r.json() : Promise.reject()).then(messages => {
        ...
        messages.slice().reverse().forEach((m, idx) => {
            ...
            row.innerHTML = `<div style="display:flex; justify-content:space-between;"><div><strong>${m.name}</strong> <div style="color:#666; font-size:13px;">${m.email} • ${new Date(m.date).toLocaleString()}</div><div style="margin-top:8px;color:#333;">${m.message}</div></div></div>`;
            el.appendChild(row);
        });
    }).catch(() => {
